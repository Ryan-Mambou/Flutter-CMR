...
function MobileNav({navLinks, toggleMobileNav, showMobileNav}) {
  ...
  return (
    <div className={`w-full h-full bg-black bg-opacity-30 fixed z-30  flex-row justify-end transition-all duration-150 ease-in ${ showMobileNav ? 'hidden' : 'flex'}`}>
        <div className={`w-80 h-full bg-white z-40 transition-all duration-300 ease-in 'translate-x-full' ${showMobileNav ? 'translate-x-full' : 'translate-x-0'}`}>
